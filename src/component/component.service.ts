...
export class ComponentService {
  ...
  async generateMonthlyReport(): Promise<string> {
    ...
    let reportOutput =
      `Olá, caro motorista! O mês virou e seu relatório chegou!` +
  ...
  private generateComponentReport(component: Component): string {
  ...
  private async verifyByUserSet(): Promise<{componentName: string; reason: string} | null> {
    const components = await this.componentRepository.find({
      where: {
        dateLastExchange: LessThanOrEqual(
          Raw((alias) => `${alias} + INTERVAL maintenanceFrequency MONTH <= CURRENT_DATE()`)
        )
      }
    })
